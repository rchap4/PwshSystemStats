...
pub extern "C" fn get_system_info() -> *mut c_char {
    ...
    match sys.get_host_name() {
        Some(h) => results.push(h.trim_matches(char::from(0)).to_string()),
        _ => results.push("unknown hostname".to_string()),
    }
    match sys.get_name() {
        Some(n) => results.push(n.trim_matches(char::from(0)).to_string()),
        _ => results.push("unknown".to_string()),
    };

    match sys.get_os_version() {
        Some(v) => results.push(v.trim_matches(char::from(0)).to_string()),
...
pub extern "C" fn get_processor_usage() -> f64 {
    ...
    let total = usage.iter().fold(0.0, |u, v| u + v);
...
/// in is null or invalid.  Pass in a valid pointer to free it exactly once.
#[no_mangle]
pub unsafe extern "C" fn disk_info_get_disk_freespace(
    disk_info_ptr: *mut DiskInfo,
    index: usize,
) -> u64 {
    // unsafe
    let disk_info = {
...
/// in is null or invalid.  Pass in a valid pointer to free it exactly once.
#[no_mangle]
pub unsafe extern "C" fn disk_info_get_disk_totalspace(
    disk_info_ptr: *mut DiskInfo,
    index: usize,
) -> u64 {
    // unsafe
...
/// in is null or invalid.  Pass in a valid pointer to free it exactly once.
#[no_mangle]
pub unsafe extern "C" fn disk_info_get_disk_usage_diskcount(disk_info_ptr: *mut DiskInfo) -> i32 {
    // unsafe
    let disk_info = {
        assert!(!disk_info_ptr.is_null()); //panic if null
        &*disk_info_ptr
